...
function FormateaNumeros($value)
{
	$numero = number_format($value, 2, ',', '.');
...
$subtotalSum += $reg_subtotal_uns[$key];

	$reg .= "['".$reg_id_uns[$key]."','".$reg_nombre_uns[$key]."', { text: '".strip_tags($reg_descripcion_uns[$key])."', fontSize: 10 },'".$reg_cantidad_uns[$key]."','".FormateaNumeros($reg_precio_uns[$key])."','".FormateaNumeros($reg_subtotal_uns[$key])."'], "; 
}
$subtotalSum1 = FormateaNumeros($subtotalSum) ;
$iva = FormateaNumeros($subtotalSum1 *12/100);
$iva_s = $subtotalSum1 *12/100;
$totalPagar = FormateaNumeros($subtotalSum+ $iva_s);
?>
<!doctype html>
...
<body>
	...
	<div class="pageWrap">
		...
		<div class="pageContent extended">
			<div class="container">
				<h1 class="pageTitle">
					<a href="#" title="#">Facturación (Ver)</a>
				</h1>
				<ol class="breadcrumb">
				...
				<div class="box rte">
					<h2 class="boxHeadline">Facturación (Ver)</h2>
...
var subtotalSum = "<?php echo $subtotalSum1 ?>";
var iva = "<?php echo $iva ?>";
var totalPagar = "<?php echo $totalPagar ?>";
...
[ '', '', '', '', '', ' '],
   [ '', '', '', '', {text: 'SUB-TOTAL:', bold: true, fontSize: 10 }, subtotalSum ],
   [ '', '', '', '', {text: 'IVA: 12%', bold: true, fontSize: 10 }, iva ],
     [ '', '', '', '', {text: 'TOTAL A PAGAR Bs.', bold: true, fontSize: 10 }, {text: totalPagar, bold: true }],
