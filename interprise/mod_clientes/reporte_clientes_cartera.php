...
$status ='';
				...
				$resulv =  mysql_query("SELECT distinct(tipo_cartera) FROM contactos_web");
				while($rowv =  mysql_fetch_array($resulv) ) { 
$status .= '<option value="';
$status .= $rowv['tipo_cartera'];
$status .= '">';
$status .= strtoupper($rowv['tipo_cartera']);
$status .= '</option>';
...
<head>
	...
	<title>Clientes por cartera</title>
...
if (isset($_GET['status'])) {
	...
	$resul =  mysql_query("SELECT * FROM `contactos_web` where anulado <> 1 and tipo_cartera = '".$_GET['status']."'");
	# code...
}
...
$resul =  mysql_query("SELECT * FROM `contactos_web` where anulado <> 1");
					...
					<tr>
					    ...
					    <td><?php echo $opciones['opciones'][$i]['tipo_cartera']; ?></td>
					    <td>
